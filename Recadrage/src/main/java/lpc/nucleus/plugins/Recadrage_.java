...
public class Recadrage_ implements PlugIn {
...
*/
	...
	public void runFile(String inputImage) {
		System.out.println("Recadrage_.runFile()");
		File currentImageFile = new File(inputImage);
		LOGGER.info("Recadrage_.runFile() Current file: {}", currentImageFile.getAbsolutePath());
		String fileImg = currentImageFile.toString();
		File outPutFileName = new File(fileImg);
		System.out.println("Recadrage_.runFile() trying calling Autocrop constructor...");
		try {
	...
	public static void main(String[] args) throws Exception {
		...
		Class<?> clazz = Recadrage_.class;
		String url = clazz.getResource("/" + clazz.getName().replace('.', '/') + ".class").toString();
		String pluginsDir = url.substring(5, url.length() - clazz.getName().length() - 6);
		System.setProperty("plugins.dir", pluginsDir);

		// run the plugin
	 	IJ.runPlugIn(clazz.getName(), "");	//IJ.runPlugIn(Recadrage_.class.getName(), "");
	}
}
